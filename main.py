...
from PyQt5.QtWidgets import (
    ...
    QFrame, QListWidget, QGridLayout, QPushButton, QStackedWidget, QTextEdit,QSizePolicy,QScrollArea,QDialog, QFileDialog
)
...
class HomeworkCard(QFrame):
    def __init__(self, title, on_click=None):
        super().__init__()
        self.setFixedSize(200, 120)  # Примерный размер
        self.setStyleSheet("""
            QFrame {
...
class HomeworkDialog(QDialog):
    def __init__(self, title, sdk, parent=None):
    ...
    def open_task(self):
        print("Открыть задание:", self.hw_title)
    ...
    def send_homework(self):
        ...
        file = getattr(self, "selected_file", None)
        print(f"Отправка: {self.hw_title}, файл={file}, коммент={comment}")
        self.accept()
        self.sdk.upload_homework(self.hw_title, file, comment)
...
class MyStatApp(QMainWindow):
    def __init__(self, sdk: MyStatSDK):
        ...
        # Контейнер с QGridLayout для карточек
    ...
    def _update_ui(self, data):
        ...
        # Добавляем карточки сеткой 3 колонки
        data_hw = self.sdk.get_homeworks_names()
        cols = 4
        row = 0
        col = 0

        for hw_name in data_hw:
            card = HomeworkCard(hw_name, lambda _, n=hw_name: self._open_hw_dialog(n))

            self.hw_container.addWidget(card, row, col)

            col += 1
            if col >= cols:  # переходим на новую строку
                col = 0
                row += 1

    def _open_hw_dialog(self, hw_name):
        dialog = HomeworkDialog(hw_name, self.sdk, self)
        dialog.exec_()

    # def _download_hw(self, hw_number):
    #     self.sdk.download_homework_by_date()
